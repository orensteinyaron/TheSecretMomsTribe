...
4. **Remotion is video-only — embedded-audio passthrough, no `<Audio>` re-overlay.** v1/v2/v3 muted `OffthreadVideo` (`volume={0}`) and overlaid the original ElevenLabs MP3 via `<Audio>`. Result: lips tracked Seedance's internal audio, ears heard the offset MP3, drift on every clip. Composition layer keeps Seedance's embedded audio canonical. **Edge-fade envelopes ARE permitted (and required — see Finding 11):** a short equal-power `volume` ramp on the passthrough audio at the fade frames of a cut is neither a full mute nor a re-overlay — mid-clip gain stays 1 and the embedded audio remains canonical. The prohibition is specifically on `volume={0}` (full mute) + a separate `<Audio>` MP3, which is what caused the lip-sync drift.
...
6. **Post-process normalization is mandatory — UNIFORM scale, position-only alignment.** Seedance produces ±150 px variation in opening face position/size from the same Soul still + same prompt (YAR-137). `normalize-clips.ts` runs AFTER `--phase=face-metrics` and BEFORE `--phase=manifest`; audio passthrough preserved via `-c:a copy`.

    **2026-06-10 fix (YAR-153, the two-backgrounds bug).** normalize originally scaled each clip by a DIFFERENT factor to equalize **face size**. Since Seedance renders Rachel at different distances, that meant a different zoom per clip — so background decor (the right-wall frames) was cropped out of the high-zoom clips and kept in the low-zoom ones, splitting the render into two visibly different backgrounds. QA missed it (`cross_clip_drift` reported `background_drift=none` — too coarse). **Fix:** normalize now applies a SINGLE uniform scale to every clip (the smallest that lets each clip's eye-line/center alignment fit, +4 % buffer), so background zoom is identical and the same decor is present in every clip. Face SIZE is intentionally NOT equalized (a slightly closer/further Rachel reads natural for UGC); only eye-line (vertical) + face-center (horizontal) are aligned by per-clip crop **translation**. Eye-line range collapses to ~2 px; face_h now varies by design. **Regression-gated** by `video/lib/background-consistency-check.ts` (`checkBackgroundScaleUniform`, unit-tested): normalize aborts if the per-clip scale ratio exceeds 1.02. Do NOT reintroduce per-clip face-size scaling. A pixel/histogram comparison of the output was evaluated and rejected as a gate — the now-intentional face-size variance dominates any region histogram (fixed 0.35 vs buggy 0.39, no separation); the scale invariant is the reliable check because all clips share one Soul start image, so uniform zoom ⇒ uniform background.
...
9. **Tail trim — IMPLEMENTED 2026-06-10 (YAR-156), was a documented-but-unbuilt "mandatory" step that shipped a defect.** Seedance frequently appends an isolated transient AFTER the last word, in the trailing silence — a mouth-click / breath / re-voicing glitch (observed on clip_05: "together" ends ~9.9 s, then a 0.28-RMS burst at ~10.05 s). Stitched, it lands in the inter-sentence gap and is heard as a "click / stitch" at the cut — the cross-fade attenuates but does not remove it (it can be louder than the speech). `normalize-clips.ts` now does a **silence-aware** trim (`video/lib/tail-trim.ts`): from the last Whisper word, scan forward to the first sustained-silence onset (the true acoustic end of speech, since Whisper word-ends under-count the decay) and cut a small pad later — before any trailing transient. A fixed `last_word.end + 0.1 s` (the old spec value) is NOT safe: Whisper under-count means it either clips the word or keeps the transient. The trimmed length becomes the clip's `whisper_duration_s` (mounted length). Video re-encodes (`libx264`); audio re-encodes too (`aac`) because the same pass bakes the edge cross-fades (see Finding 4 / 11) — a `-c:a copy` cut is not sample-accurate and misaligns the fade.
...
11. **Audio bridge = 4-frame Sequence overlap WITH an equal-power cross-fade.** Clip N+1's `<Sequence from=…>` starts 4 frames before clip N's nominal end (`AUDIO_BRIDGE_FRAMES = 4`). Both `OffthreadVideo` elements play for those 4 frames; the cut audio cross-fades (NOT a separate `<Audio>` track — that would violate Finding 4). Per-cut `bridge_enabled: false` flag exists for boundaries that sound rough; a disabled bridge becomes a hard cut with a short de-click ramp (`AUDIO_HARDCUT_DECLICK_FRAMES = 2`).

    **2026-06-10 bug + fix (the chopped-word hiccup).** As originally shipped the overlap had **no volume envelope** — both clips played at full gain, so a cut hard-spliced the outgoing word's tail onto the next clip's onset, leaving a sample-step discontinuity in the inter-sentence gap. Heard as a "chopped word" / click at the cut; worst when `bridge_enabled:false` made it a bare splice (measured sample-jump 0.025–0.098 vs ~0.0003 clean). Found by decoding the rendered audio to PCM and measuring the max sample-to-sample delta at each cut (transcription showed complete *phonemes* — the artifact is the truncated word *tail/decay*, not a missing phoneme). Fix: `AvatarV5Clip` applies an equal-power fade envelope on the passthrough audio (`audio_fade_in_frames`/`audio_fade_out_frames` from `layoutClips`) — fade-out over the overlap on the outgoing side, fade-in on the incoming side. **Regression-gated** by `video/lib/audio-boundary-check.ts`, run inside `--phase=compose`: max sample-jump at every cut must stay ≤ `AUDIO_BOUNDARY_JUMP_THRESHOLD` (0.01) or compose surfaces an AUDIO-BOUNDARY QA FAIL. Do NOT reintroduce a full-gain overlap. Lesson: **measure the audio (PCM/Whisper), never guess** — and an audio artifact at a stitch is a composition bug, not a reason to re-roll clips.
