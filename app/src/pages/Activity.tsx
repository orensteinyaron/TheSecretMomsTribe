    ...
    <div>
      ...
      {isLoading ? (
        ...
        <div className="bg-bg-surface border border-border-default rounded-lg overflow-hidden">
          <div className="grid grid-cols-[100px_120px_80px_1fr_80px] gap-2 px-4 py-2 border-b border-border-subtle">
            {['Time', 'Agent', 'Status', 'Details', 'Cost'].map((h) => (
              <span key={h} className="text-[11px] font-semibold tracking-wide uppercase text-text-secondary">{h}</span>
            ))}
          </div>
          {log.map((entry: any) => (
            <div key={entry.id} className="grid grid-cols-[100px_120px_80px_1fr_80px] gap-2 px-4 py-2.5 border-b border-border-subtle hover:bg-bg-hover items-center">
              <span className="text-xs text-text-tertiary tabular-nums">
                {new Date(entry.started_at || entry.created_at).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
              </span>
              <span className="text-xs text-text-primary font-medium">{entry.agent_name || entry.agent_slug || 'System'}</span>
