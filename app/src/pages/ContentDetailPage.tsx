...
function GenerationPanel({ context, piece }: { context: PiecePagePayload['generation_context']; piece: PiecePagePayload['piece'] }) {
  ...
  return (
    <div className="space-y-4 text-sm">
      ...
      {/* Agent Skills v1.0.0 — auditable "who/what version produced this" */}
      {(context.agent_slug || context.skill_version || context.contract_version) && (
        <div>
          <h4 className="text-[11px] font-semibold tracking-wide uppercase text-text-tertiary mb-1">Generated by</h4>
          <div className="grid grid-cols-3 gap-4">
            <StatCell label="Agent" value={context.agent_slug ?? '—'} />
            <StatCell label="Skill version" value={context.skill_version ?? '—'} />
            <StatCell label="Contract version" value={context.contract_version ?? '—'} />
          </div>
        </div>
      )}
