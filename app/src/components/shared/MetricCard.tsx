...
export function MetricCard({ label, value, change, trend, icon, onClick }: Props) {
  const trendColor = trend === 'up' ? 'text-success' : trend === 'down' ? 'text-error' : 'text-text-tertiary';
  const TrendIcon = trend === 'up' ? TrendingUp : trend === 'down' ? TrendingDown : Minus;
  return (
    <div onClick={onClick} className={`bg-bg-surface border border-border-default rounded-lg p-6 ${onClick ? 'cursor-pointer hover:bg-bg-hover transition-colors' : ''}`}>
