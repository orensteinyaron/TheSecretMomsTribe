...
**Shipped 2026-05-19.** Replaces the legacy HeyGen-based `video/scripts/generate-avatar-video.ts` (retirement deferred). Hybrid orchestration: a Claude Code session interleaves Higgsfield MCP `generate_video` calls (Seedance 2.0, `medias=[start_image, audio]`) with invocations of `video/scripts/render-avatar-full-v5.ts --phase=<name>`. State flows through `workdir/v5-state.json`. Phase sequence: **`init` → `tts` → (MCP `generate_video` + `record` + `verify` per clip) → `face-metrics` → `normalize-clips` → `face-metrics` (re-measure) → `manifest` → `compose` → `upload` → `qa` → `cover` → `summary`**. Full spec, lessons learned, and locked component designs: **[`docs/specs/AVATAR_FULL_V5.md`](docs/specs/AVATAR_FULL_V5.md)**.

### Cover stage (phaseCover) — three-asset contract (added 2026-06-11)

Every avatar piece now ships **three visual assets on the same `content_queue` row**: the video (`final_asset_url`), the thumbnail (`thumbnail_asset_url` — the first frame + purple hook banner, extracted from final.mp4 and persisted; previously transient), and a purpose-generated cover (`cover_asset_url`). The cover breaks the wall of identical frontal Rachels on the IG grid: same look/location/lighting as the reel (identity anchored to the render's Soul still as a REFERENCE IMAGE, never described in text), different expression/pose/framing.

- **External-model architecture (NOT Higgsfield):** primary tier is **Gemini Nano Banana** (`gemini-2.5-flash-image`) called directly with **`GEMINI_API_KEY` in `.env`** (key still needs to be added — services row `gemini_nano_banana` sits at `status='no_key'`). Fallback chain in the `services` table: `gemini_nano_banana` → retry with adjusted prompt → `higgsfield_soul` (Soul 2.0, session MCP; `--phase=cover` exits 5 and the session records the result via `--phase=cover-record`). Final tier logged in `metadata.cover.source`.
- **Variance rules:** `metadata.cover` stores {expression, framing, composition_side}; any combination used in the last 5 covers is excluded, framing rotates close-up → medium → three-quarter, composition drifts slightly off-center. Directive comes deterministically from concept-brief `tone` metadata when present (no LLM call), else one Haiku call.
- **Mandatory QA gate** (`video/lib/cover/qa.ts`): identity scored as "matches reference" vs the start_image (pass ≥ 3, fail → next tier), scene continuity (location/wardrobe/lighting), sameness flag vs the previous cover. Unmeasured dimensions fail closed.
- **Banner + safe zone:** the purple hook banner (brand styling: `BRAND_PRIMARY #7941EA` band, `INK #220758` shadow, Poppins ExtraBold) carries the same `hook_overlay` text as the video, and face + hook sit fully inside the IG 3:4 center-crop safe zone (rows 240–1680 on 1080×1920).
- **Persistence post-check:** one update writes both columns + `metadata.cover`, then re-reads the row and HTTP-fetches both URLs. Fails loudly.
- **Publishing:** IG Reels uses `cover_asset_url` as the cover; **TikTok keeps the frame-based thumbnail path** (its API only supports `video_cover_timestamp_ms` — documented in the smt_publisher skill).
