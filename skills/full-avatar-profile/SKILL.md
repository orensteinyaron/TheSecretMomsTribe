...
- **Build every clip's motion prompt with `video/lib/motion-prompt-builder.ts` (`buildMotionPrompt`) — do not hand-write it.** As of 2026-06-23 the builder bakes in the **aliveness floor** (natural blinking, engaged eyes, expressive micro-expressions, small head movement, "never stiff or wooden") plus, for `neutral_warm`, natural hand gestures + a smile that comes and goes. This is the fix for Rachel reading "AI-ish / a talking photo." It also bakes in a **subject-distance lock** ("same size in frame, no push-in") to counter Seedance per-clip zoom drift. Do NOT revert to a plain register-only prompt, and do NOT add "lean in" language (it reads as a zoom-in). Tone control still comes from `register`; `dry_reflective`/`urgent` stay smile-free because the floor is tone-safe.
